...
/**
 * NASNET — Active Fire & Wildfire Tracking
...
export async function GET() {
  try {
    ...
    for (const url of firmsSources) {
      try {
        const res = await fetch(url, {
          ...
          headers: { 'User-Agent': 'NASNET-Intelligence-Platform/3.5' },
        });
        if (res.ok) {
    ...
    } catch (e) { console.warn('[NASNET] Suppressed EONET error:', e instanceof Error ? e.message : e); }
