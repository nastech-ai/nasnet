...
/* ═══════════════════════════════════════════════════════════════
   NASNET — AI Intelligence Analyst Panel
...
export default function AiAnalyst({ data }: AiAnalystProps) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('nasnet-gemini-key');
    if (saved) {
  ...
  const getHeaders = useCallback((): Record<string, string> => {
    ...
    const savedKey = localStorage.getItem('nasnet-gemini-key');
    if (savedKey) {
  ...
  const saveApiKey = useCallback(() => {
    ...
    if (key) {
      localStorage.setItem('nasnet-gemini-key', key);
      setKeySaved(true);
  ...
  const clearApiKey = useCallback(() => {
    localStorage.removeItem('nasnet-gemini-key');
    setApiKeyInput('');
    setKeySaved(false);
  }, []);
              ...
              >
                ...
                <div className="flex items-center gap-2.5">
                  <div className="relative">
                    ...
                    <div className="absolute -top-0.5 -right-0.5 w-1.5 h-1.5 rounded-full bg-[var(--alert-green)] animate-nasnet-pulse" />
                  </div>
                  <div className="flex flex-col">
                    <span className="hud-text text-[11px] text-[var(--text-heading)]">NASNET ANALYST</span>
                  ...
                  >
                    ...
                    >
                      ...
                      <p className="text-[8px] font-mono text-[var(--text-muted)] leading-relaxed">
                        Your key is stored locally and sent only to the NASNET server. Get a free key at{' '}
                        <a
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1.5 mb-1.5">
                        ...
                        >
                          {msg.role === 'user' ? 'OPERATOR' : 'NASNET ANALYST'}
                        </span>
